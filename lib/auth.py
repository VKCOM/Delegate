...
class Connector(Module):
    def __init__(self, server1, socket):
        ...
        self.__request = None
        self.__commands = {
            b'test': self.__cmd_test,
            b'hello': self.__cmd_hello
        }
    ...
    def __run(self, command_key, command_hash, command_run):
        ...
        real_hash = hashlib.sha256(
            b':'.join([
                real_key, self.salt2, self.__hash, b'%'.join(command_run)
            ])
        ...
        self.__request = Request(
            command_key, command_run[0], command_run[1:], self
        )
    ...
    def __cmd_hello(self, arguments):
        self.__hash = hashlib.sha256(
            self.salt1 + self.__salt_random + (
                ":%d_%d" % (self.__id, self.__local_id)
            ).encode("ascii")
        ).hexdigest().encode("ascii")
        self.__local_id += 1
