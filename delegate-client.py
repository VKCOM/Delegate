...
parser.add_argument(
    "target",
    help="host and port of delegate server you want to connect",
    metavar="host:port"
)
parser.add_argument(
    "script",
    help="name of the script you want to launch"
)
parser.add_argument(
    "-i",
    help="override default path to passfile",
    metavar="path/to/passfile"
)
parser.add_argument(
    "params",
    help="parameters to script if applicable",
    metavar="params",
    nargs="*"
)
...
key_id, key_value = \
    open(path_to_passfile).readline().strip().encode().split(b":")
...
try:
    ...
    started = query(
        b'run ' + key_id + b' ' + command_hash + b' ' + b' '.join(command)
    )
    queued = None
